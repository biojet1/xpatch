...
def patchDB(patfile, ctx):
    ...
    if ctx.bVerbose:
        for alter in patches:
            ...
            say2("-------------")
            say2("Patch:", alter["id"])
            say2("Range: 0x%X - 0x%X (%d - %d)" % (start, end, start, end))
            say2("Size : %d (0x%X) bytes" % (length, length))
            say2(
                "From :",
                hexlify(dataFrom[:31]).decode()
                + "%s (%d)" % (lenFrom > 31 and "..." or "", lenFrom),
            )
            say2(
                "To   :",
                hexlify(dataTo[:31]).decode()
                + "%s (%d)" % (lenTo > 31 and "..." or "", lenTo),
            )
            # say2("-------------");
    return meta, patches
...
def patch(patches, target, ctx):
    ...
    for alter in patches:
        ...
        data = f.read(length)
        # if ctx.bVerbose:
        # 	say2("data :", length, len(data), hexlify(data));
        # 	say2("data :", len(dataFrom), hexlify(data[0 : len(dataFrom)]));
        # 	say2("data :", len(dataTo), hexlify(data[0 : len(dataTo)]));
        alter["use"] = 0
        if len(data) != length:
            raise RuntimeWarning(
                "Invalid length %d read for `%s', %d expected "
                % (len(data), id, length)
            )
            ...
            say2("Invalid:", id, hexlify(data[:31]).decode())
    f.close()
    if use > 0:
...
def main():
    ...
    def check_md5(file, md5):
        ...
        if fmd5 == md5:
            ...
            say2("Error: Invalid MD5 %s != %s" % (fmd5, md5))
    ...
    ctx.add_option(
        "-m", action="store_true", dest="bVerify", help="check md5 digest if given."
    )
    ...
    if len(args) > 0:
        ...
        while len(args) > 0:
            target = args.pop()
            say2("Source:", target)
            (not ctx.dryRun) and (not ctx.unPatch) and ctx.bVerify and meta[
                "md5"
            ] and check_md5(target, meta["md5"])
            patch(patches, target, ctx)
            (not ctx.dryRun) and ctx.unPatch and ctx.bVerify and meta[
                "md5"
            ] and check_md5(target, meta["md5"])
